...
import { collection, query, where, orderBy, onSnapshot, getDocs, doc, addDoc, serverTimestamp, Timestamp } from 'firebase/firestore';
...
import { PlusCircle, Loader2, Camera, X, ChevronLeft, ChevronRight } from 'lucide-react';
import {
  Carousel,
  type CarouselApi,
  CarouselContent,
...
const StoryCreator = ({ open, onOpenChange, onStoryCreated }: { open: boolean, onOpenChange: (open: boolean) => void, onStoryCreated: () => void }) => {
    ...
    const handleCreateStory = async () => {
        ...
        setIsSaving(true);
        try {
            const userPublicProfileRef = doc(db, 'publicUsers', user.uid);
...
const StoryViewer = ({ open, onOpenChange, userStories }: { open: boolean, onOpenChange: (open: boolean) => void, userStories: Story[] | null }) => {
    ...
    useEffect(() => {
        if (!open) return;
        // Reset progress when a new story is selected
        setProgress(0);
...
export function StoriesSection() {
    ...
    const sortedUsersWithStories = useMemo(() => {
        return Array.from(storiesByUser.keys()).sort((a, b) => {
            ...
            return 0; // Or sort by most recent story time
        });
                                ...
                                return (
                                    <div key={userId} className="flex flex-col items-center gap-2 cursor-pointer" onClick={() => handleViewUserStories(userId)}>
                                        <Avatar className="h-16 w-16 border-2 border-primary p-0.5">
                                            <AvatarImage src={userDetails.userPhotoUrl} />
